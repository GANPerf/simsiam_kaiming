...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = resnet50().cuda(args.gpu)

    classifier = Classifier(2048, args.class_num).cuda(args.gpu)


    # freeze all layers but the last fc因为论文说了，在测试模型的时候，前面的特征冻住，只训练后面的fc的参数。1000正好和imagenet1000对应上了。。。
    for name, param in model.named_parameters():
        if name not in ['fc.weight', 'fc.bias']:
            param.requires_grad = False

    model.fc = nn.Linear(2048, args.class_num).cuda(args.gpu)  ##############
    # init the fc layer
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        target = target.cuda(args.gpu, non_blocking=True)#batch

        # compute output
        #images = Variable(images, requires_grad=True)
        _, feat = model(images)#batch*2048################################
